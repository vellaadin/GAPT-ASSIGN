...
async function startWebcam() {
  ...
  if (accessGranted) {
    // Inform the user that access is already granted
    console.log("Access already granted!"); 
    // Alert the user for immediate attention
    alert("Access already granted!");  // More user-friendly message
    return;
  }  
  ...
  try {
    ...
    // Inform the user about the webcam access issue
    displayModal("Oops! It seems we couldn't access your webcam. Please make sure it's not being used by another application."); // More user-friendly message
    // Log the error for debugging purposes
    console.error("Couldn't access the webcam:", error);
    return;
  }  
...
async function loop() {
  ...
  if (running) {
    ...
    selectMusic(); // Select music based on the detected emotion if running
  }
...
function startMusicSelection() {
  ...
  if (!accessGranted) {
    displayModal("Please allow access to your webcam first!"); // More user-friendly message
    return;
  }
...
function toggleMusicSelection() {
  if (running) {
    stopMusicSelection();
    updateMusicMessage(mostLikelyEmotion);
  } else {
    startMusicSelection();
  }
}
