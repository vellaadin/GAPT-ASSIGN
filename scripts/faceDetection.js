...
function selectMusic() {
  ...
  // Logic for selecting music based on emotion
  switch (mostLikelyEmotion) {
    case "Sad":
      fetchPlaylistAndPlay("Sad");
      break;
    case "Surprise":
      fetchPlaylistAndPlay("Surprise");
      break;
    case "Neutral":
      fetchPlaylistAndPlay("Neutral");
      break;
    case "Happy":
      fetchPlaylistAndPlay("Happy");
      break;
    case "Fear":
      fetchPlaylistAndPlay("Fear");
      break;
    case "Disgust":
      fetchPlaylistAndPlay("Disgust");
      break;
    case "Angry":
      fetchPlaylistAndPlay("Angry");
      break;
    default:
...
// Function to fetch playlist based on emotion and play a random track
function fetchPlaylistAndPlay(emotion) {
  fetch(`../Playlists/${emotion}/${emotion}.json`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`Failed to fetch ${emotion} playlist`);
      }
      return response.json();
    })
    .then(data => {
      const randomTrack = getRandomTrack(data);
      const url = randomTrack.url;
      updateMusicPlayer(url); // Call function to update music player with the selected URL
    })
    .catch(error => {
...
// Function to update the music player with the selected URL
function updateMusicPlayer(url) {
  const musicPlayerIframe = document.querySelector("#musicPlaceHolder iframe");
  if (musicPlayerIframe) {
    musicPlayerIframe.src = url;
  }
}
...
// Function to update the music player with the selected URL
function updateMusicPlayer(url) {
  // Assuming you have an iframe element with an id of 'musicPlayer'
  var iframe = document.querySelector("#musicPlaceHolder iframe");
  // Set the src attribute of the iframe to the selected URL
  iframe.src = url;
}
